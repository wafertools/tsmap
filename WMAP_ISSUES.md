...
| testdata-parser version | **0.10.1** — patch bump adopted 2026-09-12 (tsmap 0.1.35). **0.10.0** was published to npm and adopted (2026-09-11, bumped from **0.9.0**): STDF WCR read by the spec (it was 2 bytes out of place), ATDF read in its own field order, the V4-2007 VUR record read, several lot records per stream labelling each wafer with its own lot, CSV/JSON/Parquet rows grouped by lot + wafer (`flat_wafers.rs`), and `parquet_distinct_count` for the file filter's Parquet wafer count. `npm run parser:unlink` restored the published package, `package.json`/`package-lock.json` pin `^0.10.0`. |
| Last updated | 2026-09-17 (wmap bumped **0.30.0 → 0.30.1** in tsmap, alongside this repo's own batch — see tsmap's `CHANGELOG.md` [0.1.37]. Published, unlinked, pinned to `^0.30.1`. tsmap-side changes: `downloadFilename` removed from both render calls, `src/icons.ts` deleted in favour of wmap's `ICONS`, the file-filter dialog title no longer shows a stale count, and the file-filter capture step drives the `webkitdirectory` fallback so its screenshots refresh again.) |
| Previous update | 2026-09-15 (wmap bumped **0.29.0 → 0.30.0** in tsmap, alongside this repo's own batch — see tsmap's own `CHANGELOG.md` [0.1.36]. Published, unlinked, pinned `^0.30.0`. Issues #56, #57 and #58 closed by this bump. #58 was found while preparing the release itself, from a user report with `sample_data/NO-WAFER-ALL-COORDLESS-01.csv`, and fixed in wmap before publishing. The previous bump (0.28.0 → 0.29.0, tsmap 0.1.35, 2026-09-12) was recorded only in the "wmap version in use" row, not here.) |
...
### 16. ~~Toolbar icon set is internal — host cannot import it to match wmap's iconography~~ (fixed in wmap 0.26.0)

**Rechecked 2026-09-17:** wmap exports `ICONS` from `/render` (`canvas-adapter/index.ts`) since 0.26.0. tsmap still carries its own copies in `src/icons.ts`, and uses three of them: `close`, `maximize` and `shrink`. All three match wmap's `close`, `maximize` and `minimize` exactly, so on 2026-09-17 tsmap switched to importing them: `modal.ts` and `testSelectorUI.ts` now import `ICONS` from `@wafertools/wafermap/render`, using `minimize` where tsmap's copy called it `shrink`, and `src/icons.ts` is deleted. Its other four icons were unused, including tsmap's own `printer`.
...
### 24. ~~In-app user-guide modal has no print / save-as-PDF affordance~~ (fixed in wmap 0.26.0)

**Rechecked 2026-09-17:** the guide window has a Print / Save as PDF header button (`OverlayOptions.printable`, `toolbar.ts`), shipped with #37's batch in 0.26.0.
...
**Related, fixed 2026-09-17 (wmap 0.30.1):** in tsmap a detached card opens in this in-page fallback window. Maximized, its header sits directly under tsmap's OS title bar, so the app showed two rows of minimise/maximise/close whose minimise buttons did different things. The in-page button is now **Collapse** (chevron icon, tooltip "Collapse" / "Show contents") and is hidden while the window is maximized, leaving restore and close. The two rows only go away completely with this issue's real fix: OS windows in Tauri. No tsmap code change.
...
### 28. ~~`renderWaferGallery` does not propagate `onSaveImage` to per-card renderers — per-card PNG save falls back to detached anchor~~ (fixed in wmap 0.18.0 or earlier)

**Rechecked 2026-09-17:** both per-card render paths in `renderWaferGallery.ts` (grid card and detached/expanded card) pass `onSaveImage` and `onSaveText`. They have done so since at least v0.18.0.
...
### 31. ~~wmap↔tsmap boundary rework — moving reusable chart/analysis code into wmap, plus a wmap-owned "Analysis" tab~~ (complete; one optional idea left)

**Rechecked 2026-09-17:** the deferred items below are resolved. Yield and boxplot bars open the wafer (`openWafer`, #51, 0.28.0). Findings highlight their dies when clicked. The charts stay under `canvas-adapter/charts/`, not a subpath (see wmap's CLAUDE.md). One item is left, as a feature idea rather than a defect: pareto and bin-cluster bars are deliberately not clickable. They would need a lot-stack map of the clicked bin.
...
### 34. ~~No first-class per-test pass/fail datum — functional (FTR) tests were faked as values and got parametric statistics~~ (fixed in wmap 0.20.3; tsmap side shipped with testdata-parser 0.5.0)
...
### 35. ~~Linux Wayland/Tauri hidden-window decoration bug — tsmap host workaround~~ (not a wmap issue)

**Rechecked 2026-09-17:** this is a Tauri/GTK issue, not a wmap gap. It was logged here only for the record. The workaround is still needed and still present: `src-tauri/src/lib.rs` toggles `set_resizable` on focus, and `tauri.conf.json` still creates the window hidden. Remove the workaround when an upstream Tauri/TAO release fixes the bug.
...
### 37. ~~Summary reports open via `xdg-open` into an existing, unfocused browser window — looks like the button silently does nothing~~ (fixed in wmap 0.26.0)

**Rechecked 2026-09-17:** reports open in wmap's in-app modal (`openReportModal`). `setReportOpener` → `platform.openReport` → `xdg-open` is now only the explicit "Open as full page" link. tsmap points 1, 2 and 4 are done, and 3 (publish) shipped as 0.26.0. Nothing below records whether the print dialog was confirmed under WebKitGTK. Check that manually when testing the next Tauri build.
...
### 55. ~~Map-space stroke widths are in DEVICE pixels — every overlay line thins as display scaling rises~~ (fixed in wmap 0.30.1)

**Fixed 2026-09-17 (wmap 0.30.1):** implemented as suggested. `toCanvas.ts` now sizes every map-space stroke and marker in CSS pixels (`cssPx = 1 / ppm`). The die outline is the one exception and stays a device-pixel hairline (`devicePx`), because measuring showed CSS-pixel outlines grey out a dense map on high-DPI screens (0.8 mm pitch, 520 px box: mean luminance 205 → 189 at dpr 2). Measured with Chrome at dpr 1/2/3: dpr 1 is pixel-identical to before, and at dpr 2 and 3 the ring and quadrant lines are about the CSS width they have at dpr 1 (previously ½ and ⅓). Guarded by `tests/strokeScale.test.mjs`, which fails on the old code.
...
### 59. ~~Plot-mode "Test Value ▶" submenu cannot be opened on a touchscreen — no test can be chosen on a phone~~ (fixed in wmap 0.30.1)

**Where:** wmap `packages/canvas-adapter/toolbar.ts`, `buildModeMenuEl`'s cascade row, and `closeOpenMenu`.

**Problem (found 2026-09-17, tsmap web build on a phone):** with more tests than `INLINE_TEST_LIMIT` (6), the toolbar's plot-mode menu puts them in a "Test Value ▶" submenu. That submenu opened on `mouseenter` only, and the row's click handler did nothing. A touch tap sends an emulated `mouseenter` and then a `click`. The `mouseenter` opened the submenu and registered a `{ once: true }` document click listener to close it, and the same tap's `click` fired that listener, so the submenu closed immediately. Other list selectors worked because their rows are tapped directly. Enter on the row did nothing from the keyboard either. Separately, the submenu is a sibling of the menu, so `closeOpenMenu` treated a tap on its filter box as outside and closed the menu underneath.

**Fix (wmap 0.30.1):** the row opens the submenu on click as well as hover, which covers taps and Enter/Space. The submenu's outside-click listener ignores the row and the submenu, and is removed on close. Focus moves into the submenu when it is opened from the keyboard, the arrow keys navigate it and Escape closes it. The submenu is marked `data-wmap-submenu`, which `closeOpenMenu` treats as inside the menu. Confirmed in Chrome's Pixel 7 emulation with real taps: on the old code the submenu is gone after the tap, and on the new code a test can be tapped and becomes the plot mode. Guarded by `tests/modeMenuCascade.test.mjs`, which fails on the old code.
