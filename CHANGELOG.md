...
- **Test-value findings now run automatically when they are cheap, and announce themselves when they are not.** The regional test-value analysis lived only as `Lot ▾ → Show test-value findings`, off by default. Two problems: a reader looking at the Findings panel had no signal that a whole category of finding had been skipped, and nothing would ever send them into that menu to discover it — so most users never saw the analysis at all. Advertising the control could not fix an invisible absence. tsmap now estimates the cost (`src/valueFindings.ts`) and, below a 1s budget, simply runs it and says so in the log; above that it passes wmap a `FindingsNotice` (new in wmap 0.27.0) which draws a row at the top of the Findings section reading "Test-value findings are not included — regional analysis of *N* tests across *M* wafers, about *T*" with an **Analyse** button. The offer and its price arrive together, in the one place the reader is already looking. The cost model is `wafers × dies × tests × 1.2µs`, measured against wmap's own analysis across 5–25 wafers, 10.7k dies and 30–100 tests, where the pass ran 1.9s / 6.7s / 9.7s against a 0.3–1.4s baseline; `src/valueFindings.test.ts` pins it to within a factor of two of those figures, and asserts that `testdata/correlated.csv` (5 wafers, 9,865 dies, 30 tests ≈ 0.35s) lands under the budget while a 25-wafer production lot lands over it. The menu item stays — it is now how you turn the analysis back *off* — and either route marks the choice as the user's, after which the budget stops deciding for them. It remains opt-in above the budget on purpose: defaulting it on would make a large lot feel broken, and a 25-wafer lot with a few hundred tests takes tens of seconds.

### Added

- **Help → About tsmap…** — version and build date, author, licence, and links to tsmap, wafermap and the wafertools site. The app previously stated its version only as a line in the log panel, and stated its authorship and licence nowhere at all. Attribution sits here rather than on the toolbar or a splash screen because MIT already binds the copyright notice to every copy: this is for the person who goes looking, not something to put in front of someone opening a wafer map. The wafermap link is deliberately described as the library this is built on and usable in your own application, so a developer who arrived at the app but actually wants the engine has a route out. `package.json` gained the `author` field it was missing, matching wafermap's.
...
- **`check-clones.mjs`** — structural duplicate detection, wired into `check:docs`. Ported from wafermap, where it found four cross-file clones including two that were correctness risks. It compares shape rather than names: identifiers and literals are normalised away and equal-length token windows are matched across files, so code that was copied and then renamed still matches. tsmap comes back clean at the shipped 150-token threshold; the only things below it are `splitsUI` and `testSelectorUI` both implementing the host adapter for `createRangeSelection`, which resemble each other because they conform to one interface — the pattern working, not a clone, and a useful confirmation the threshold sits in the right place.


- **Fourteen off-scale spacing values snapped to the scale.** `3`, `5`, `14`, `18`, `20` and `22px` across `index.html`'s stylesheet, the splits dialog, the test selector, the definitions dialog and the modal base — each defensible alone, collectively a scale nobody was following. Snapped to their nearest step (`3`→`4`, `5`→`6`, `14`→`12`, `18`/`20`→`16`, `22`→`24`), which also unified two dialogs that were padded `16px` and `20px` for no reason. The check's budget is now zero rather than the count it started at, so the next one has to be argued for.
...
- **`check:themes` pins the documented theme list to `THEME_GROUPS`.** The picker went from 8 themes to 16 this release, which meant editing `theme.ts`, sixteen CSS blocks, the user guide and CLAUDE.md — and nothing tied the last two to the first. The new check compares **names and order**, not a count: a count catches "8 became 16" and misses a rename, a reorder, or a theme moved between the Light and Dark groups, all of which make the guide wrong in the way a reader actually notices, since the guide is what they scan to find the theme they want. It also verifies the spelled-out count in CLAUDE.md. Wired into `check:docs`, and verified by breaking it three ways — adding a theme without documenting it, renaming one in the docs only, and reordering the documented list.
...
- **A multi-file load flattened every file's test definitions into one list, so wafers were plotted against another file's spec limits.** `main.ts` merged each file's `testDefs` with `Object.assign`, which is last-wins, and handed that single object to wmap for *every* wafer. A test number identifies a test **within one test program**, so across files it is not an identity at all — and tsmap's own fixtures prove it: test 1001 is `vth_n_mV` (mV, 260-380) in `PVT-LOT-05.stdf`, `leakage_nA` (nA, 0-5) in `COORDLESS-LOT-01.stdf` and `leakage` (nA, 0.5-5.5) in `EDGE-LOT-01.stdf`. Loading them together silently discarded two of the three definitions, then plotted, labelled, normalised and capability-scored all three lots against the survivor's — a box plot of nanoamp readings titled `vth_n_mV` with a `USL 380` line across it, a process-capability panel reporting **Ppk −1489**, and drilled-in maps showing every die out of spec. Nothing warned, because by the time the data reached wmap every wafer carried an identical, already-wrong list.

  Each wafer now carries **its own file's** definitions (`RenamedWafer.testDefs`, `currentDefsBySource`, keyed on the `WaferSource` its file's wafers already shared by reference), so wmap reconciles them per wafer and withholds any test number the files disagree about. `currentTestDefs` remains the lot-wide union for what legitimately needs one — the test selector, the test-definitions file, `applyTestSelection` — and is built by a new `unionTestDefs` (`lib.ts`) that reports disagreements instead of silently resolving them. **An absent field is "not stated", never a disagreement:** loading a file that carries limits alongside one that does not is an ordinary, valid combination and merges silently. Only two *stated and different* values collide — a different name, unit or test type means different measurements and the test is withheld from every cross-wafer chart, report and map mode (logged at error, naming both files); different spec limits keep the test and its distributions but withhold capability, spec yield and the limit lines (logged at warning). Limits compare on a relative tolerance rather than exact equality, so a float32 STDF limit and a float64 CSV one cannot manufacture a conflict, and names compare trimmed and case-insensitively so `TEST_TXT` drift cannot withhold data.

  Test-definition overrides now reach the per-file lists as well as the union — applying them to the union alone would have stopped them reaching the map entirely. An override for a number the files disagree about is refused and named, since one definition cannot honestly describe two measurements; a *limits* disagreement is deliberately still accepted, because stating the spec explicitly resolves the ambiguity rather than hiding it. Requires wafermap ≥ 0.27.0 for the per-wafer reconciliation (`mergeTestDefs`) and its two new warnings. See `WMAP_ISSUES.md` #50.

- **The first-pass test scan flattened test definitions across files too, so the selector named a test after whichever file scanned last.** `scanBinaryTests` merged each file's scan result with `Object.assign(merged, result.testDefs)` under a comment asserting "test numbers are the identity key" — the very assumption that does not hold across test programs. The selector then listed test 1001 once, under one of several names, and the user chose it without being told the files disagree. Selection is by test *number*, so the imported data was never wrong; the name it was chosen by was one of several. It now goes through `unionTestDefs`, which gives a deterministic first-wins list and reports the disagreement **at scan time — before the choice**, rather than after the load.

- **A collision report named only the first disagreeing pair, which described the mildest disagreement and hid the worst.** With three lots defining test 1001 as `leakage` (nA), `leakage_nA` (nA) and `vth_n_mV` (mV), the log said only that the first two disagreed — reading as though the third file were not involved, and understating a nA-versus-mV clash as a naming quibble. `TestDefCollision` now carries every distinct stated definition with the first file to state it (`stated`, replacing the `values`/`files` pairs), a later file restating one already named adds nothing, and a name disagreement outranks a limits one on the same number.

- **A multi-file load unioned every file's pass hard bins, which could silently turn a fail bin into a pass bin lot-wide.** `mergePassHbins` unions, so a hard bin marked Pass by one file's HBR became a pass bin for *every* wafer in the load — including wafers from a file that counts it as a fail. Unlike the bin *name* merge beside it, this is not cosmetic: it moves the yield of every wafer the bin appears on, and with it the findings, the region yields and the report. Each file's wafers are now judged by that file's **own** pass bins (`FileDefs.passHbins`, carried per wafer alongside its test defs), and a disagreement is logged naming both files and the bin.

  STDF records only the pass flag (`if hbr.pass { pass_hbins.insert(…) }`), so "this file calls bin 5 a fail" is not stored anywhere — `unionBinInfo` (`lib.ts`) recovers it from the bin appearing on that file's own dies (or in its own HBR names) while absent from its pass list, which is exactly how that file's own yield is computed, and needs no parser change. Absent is not an assertion here either: a file that states **no** pass bins is saying nothing and inherits the lot-wide union, rather than falling back to wmap's `[1]` default, which would itself have reclassified its dies. Bin *names* are still merged lot-wide — that one is cosmetic and touching it would change the gallery legend.

- **A re-parse after changing the test filter produced wafers with no provenance at all.** The "Filter tests…" re-parse path built its wafers straight from the parsed files without stamping a `WaferSource`, so those wafers lost their per-file WCR geometry (`wcrFor` fell through to the no-source entry) — and would have lost their per-file test defs too. It now stamps one source per file, the same shared-by-reference way the load path's `buildRenameRows` does.

- **Vite's dependency scan crawled every `.html` under the project root.** 157 files here: the generated docs site (`site/`), the packaged guide (`public/guide/`), and dozens of Tauri codegen assets and old `.deb` bundle trees under `target/`. `optimizeDeps.entries` now names `index.html`, the only real entry.

  This is **not** covered by `server.watch.ignored`, which was already excluding `**/target/**` for exactly the same "large, high-churn, irrelevant to the frontend" reason. That bounds the file watcher; the dependency scan is a separate setting and had to be told separately — the two read like they should be one thing and are not.

  Scoped as housekeeping, **not** as the fix for the "Failed to scan for dependencies … write EPIPE" that prompted it: bounding the crawl to one entry did not stop that error. EPIPE there is esbuild's child process being killed, and the machine it appeared on was out of swap. Recorded here so the next reader does not mistake this for a cure.


- **A mixed-format batch could become impossible to load, with a refusal that named files you could not see.** "Select none" cleared only the rows currently passing the search/column filter, so narrowing a mixed batch to one format left the others selected and unreachable: the count read "4 selected / 2 shown", the load was still refused as mixed, and no control on screen could clear the offenders. The only escape was guessing the search box was responsible.

  `selectNone` now clears the whole selection. The asymmetry with `selectAll` (still scoped to the shown rows) is deliberate and forced: selection deliberately survives a filter change, because ticking files across two different searches is a real workflow — and once hidden rows can stay selected, a scoped "none" is guaranteed to be able to strand them, while an unscoped "all" would select thousands of unseen files. Both cannot be scoped without recreating this bug.

  Because that asymmetry is invisible on its own, the state it produces is now stated: the count reads `4 selected (2 hidden) / 2 shown / 4 total` whenever the selection reaches beyond the filter, and the mixed-format refusal says how many selected files are hidden and which control clears them. The pre-existing scan summary ("Scanned 4 files — 2 formats — a load must pick just one") is a fact about the scan and is unchanged; it is not a live warning and never cleared, which is part of why the refusal looked stuck.

  No unit test: `buildFilterTable` builds real DOM and this suite runs in node with no jsdom configured, so the behaviour is covered by a scripted end-to-end reproduction rather than a unit test — a real gap, noted rather than papered over.


- **Six clickable things did not react to being pointed at, and a consolidated style constant had never actually been deleted.** Found by a new hover rule in `check-style-scales.mjs` (see wafermap's changelog for the rule itself): a `cursor: pointer` promises interactivity, and an element that offers the promise and does nothing reads as dead. The six were the checkbox rows in the column picker, the value filter and the file-associations dialog, the splits dialog's suffix label and its `.split-label` rows, and an inline link — all now covered by two small shared classes (`.click-row`, `.link-inline`) rather than per-site styling, alongside the blanket `button:hover` rule that already covered every button.

  The same rule surfaced `SECONDARY_BTN_CSS` in `modal.ts`. `index.html`'s own comment states "The constant is gone; this is the single definition" — describing the consolidation onto `.btn-secondary` — but the constant was still exported, unused, and was specifically the copy **without** a hover state, which was the whole reason for consolidating. Anyone importing it would have got back the defect the consolidation removed. Now actually deleted, so the code and the comment agree.


- **Scanning a folder in the BROWSER build failed every file with "file too short to contain a FAR record".** The scan is deliberately lazy — `webPlatform.pickFolder` returns handles with an empty `bytes` and the originating browser `File` parked on `webFile`, so pointing it at a large folder reads nothing until something asks. `materializePicked` is what finally reads those bytes, but it looks at `PickedFile.blob`; `pickedFromHandle` returned a bare `{ handle }` and dropped `webFile` on the floor, so every scanned file reached the parser with zero bytes. The parser then reported the most literal thing it could see — a file too short to hold a FAR record — which reads as corrupt data rather than as data that was never loaded. `webFile` is now declared on `FileHandle` (it was an undeclared excess property) and carried into `blob`, with the coupling stated at both ends.

  The capture harness now falls back to the system Chrome on its own (`scripts/lib/browser.mjs`), matching what `../wafermap` already did. Playwright's bundled Chromium cannot be installed on this machine at all — `npx playwright install chromium` refuses with "Playwright does not support chromium on ubuntu26.04-x64" — so its "download new browsers" banner is advice that cannot be followed, and every capture run dead-ended until someone set `CHROME_PATH` by hand. `CHROME_PATH` still forces a specific binary; it is no longer needed for an ordinary run. The original launch error is rethrown if the fallback also fails, so a genuinely missing browser still reports why the normal path failed.

  Desktop was never affected: Tauri handles carry `path` and the parser reads that. Introduced by the same (unreleased) commit that added folder scanning, so no published build ever carried it — folder scanning is new in this release and the defect was caught before it left the tree. It survived in the working tree only because the one artefact that would have shown it — the file-filter screenshot — had *also* broken in that commit, so it silently kept publishing the pre-feature image. The capture drove a `webkitdirectory` input with a list of individual files, which Chromium now refuses ("requires passing a path to a directory"); it stages the requested files into a temp folder and scans that instead, so the caller's file list still means something rather than the whole 17-entry `testdata/` being swept in. Covered by `src/pickedFile.test.ts`, verified by reintroducing the one-line defect and watching two of its three cases fail.
